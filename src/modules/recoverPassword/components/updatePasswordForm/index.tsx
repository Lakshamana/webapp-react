...
const UpdatePasswordForm = ({ handleFormSubmit, dispatchError, error }: Props) => {
    ...
    return (
        <CardContainer paddingX={[30, 60]} paddingY={[40, 40]} width={[1, sizes.loginCardWidth]}>
            <Text fontSize={24} textAlign={'center'} fontWeight={'bolder'} color={colors.generalText[colorMode]}>{t('updatePassword.title')}</Text>
            <Text fontSize={16} paddingTop={10} textAlign={'center'} color={colors.secondaryText[colorMode]}>{t('updatePassword.subtitle')}</Text>
            <>{!!error && <AlertComponent paddingTop={20} type={'error'} description={error} onClose={dispatchError} ></AlertComponent>}</>
            <Flex alignItems={'center'} marginTop={30} flexDirection={'column'} gridGap={3}>
                <Input
                    name="oobCode"
