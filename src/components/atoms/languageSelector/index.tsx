...
const LanguageSelector = () => {
    ...
    const selectedLanguage: LanguageSelectProps = languageOptions.filter((item) => {
        ...
        <Menu>
            ...
            <MenuList minWidth={'auto'} bgColor={'#444444'} border={'none'} color={'white'} padding={0}>
                {languageOptions.map((item, idx) => {
                    return <MenuItem _focus={{ bg: "transparent" }} paddingRight={8} icon={item.flagIcon} key={idx} minH="48px" fontWeight={500} onClick={() => handleChangeLanguage(item)}>
